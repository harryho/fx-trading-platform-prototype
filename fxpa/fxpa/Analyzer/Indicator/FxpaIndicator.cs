...
namespace fxpa
{
    public class FxpaIndicator : BasicIndicator
    {
        /// <summary>
        ...
        /// 
        /// </summary>
        FxpaIndicator(string name, string description, bool? isTradeable, bool? isScaledToQuotes, string[] resultSetNames)
            : base(isTradeable, isScaledToQuotes, resultSetNames)
        {
            this.Name = name;
            this.Description = description;

            _ui = new FxpaIndicatorUI(this);
        ...
        static Double[] method(Double[] inReal)
        {
        ...
        protected override void OnCalculate(int startingIndex, int indexCount)
        {
            ...
            lock (this)
            {
               ...
               //     parameters.Add(GetInputArrayValues(info.Name, startingIndex, indexCount));
                    parameters.Add(DataProvider.DataUnits);
                }
                ...
                // outNBElemenet
                parameters.Add(DataProvider.DataUnits.Count);
            ...
            int k = 0;
            foreach (BarData bar in (List<BarData>)parameters[2])
            {
                ((double[])parameters[parameters.Count - 1])[k] = fxi.handleFullCandle(bar);
                k++;
            }
